...
export class CategoryFilterComponent implements OnInit {
  ...
  ngOnInit() {
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      this.activeParams = ActiveParamsUtil.processParams(params); //сходили в утилиту за причесыванием query-параметров

      if (this.type) {
        if (this.type === 'height') {
          this.open = !!(this.activeParams.heightFrom || this.activeParams.heightFrom); //!! переводит в boolean
          ...
          this.to = this.activeParams.heightTo ? +this.activeParams.heightTo : null;
            ...
            this.open = !!(this.activeParams.diameterFrom || this.activeParams.diameterFrom); //!! переводит в boolean
        ...
        if (this.categoryWithTypes && this.categoryWithTypes.types && this.categoryWithTypes.types.length > 0 &&
          ...
          // })
          // some отдаст true при первом же найденном совпадении (при наличии в url активного параметра)
          // можно реализовать двумя циклами
          this.open = true;
        }
      }
    });
